...
void get_current_floor(RendererSimObs *s){
    ...
    ripl_map_request_msg_t_publish(s->lcm, "FLOOR_STATUS_REQUEST", &msg);
}
...
static void floor_status_handler(const lcm_recv_buf_t *rbuf, const char *channel,
                                 const ripl_floor_status_msg_t *msg, void *user)
{
...
gboolean on_transmit_obs(gpointer _user)
{
    ...
    /*ripl_floor_rect_list_t floor_rects;
    ...
    //ripl_floor_rect_list_t_publish(self->lcm, "FLOOR_SIM_RECTS", &floor_rects);
    free(rects->rects);
...
renderer_simobs_new (BotViewer *viewer, lcm_t *_lcm, BotParam *_param )
{
    ...
    */
    ...
    ripl_floor_status_msg_t_subscribe(self->lcm,"CURRENT_FLOOR_STATUS",floor_status_handler,
                                       self);
