...
void get_current_floor(RendererSimObs *s){
    ...
    maplcm_map_request_msg_t_publish(s->lcm, "FLOOR_STATUS_REQUEST", &msg);
}
...
on_sim_rects_new(const lcm_recv_buf_t *rbuf, const char *chennel,
                 const obs_rect_list_t *msg, void *_user)
{
    ...
    memcpy(self->rects->data,msg->rects,sizeof(obs_rect_t)*self->rects->len);
...
static int find_idx(RendererSimObs *self, const double xy[2])
{
    ...
    for (unsigned int i = 0; i < self->rects->len; i++) {
        obs_rect_t *rect = &g_array_index(self->rects, obs_rect_t, i);
...
static void floor_status_handler(const lcm_recv_buf_t *rbuf, const char *channel,
                                 const maplcm_floor_status_msg_t *msg, void *user)
{
                         ...
                         const double ray_start[3], const double ray_dir[3], const GdkEventMotion *event)
{
    ...
    obs_rect_t *rect = &g_array_index(self->rects, obs_rect_t, self->edit_idx);
                         ...
                         const double ray_start[3], const double ray_dir[3])
{
    ...
    obs_rect_t *rect = &g_array_index(self->rects, obs_rect_t, self->hover_idx);

    return sqrt(SQ(self->rects_xy[0] + rect->dxy[0] - xy[0]) +
                SQ(self->rects_xy[0] + rect->dxy[1] - xy[1]));
}

static void draw_rect (RendererSimObs *self, obs_rect_t *rect, int draw_orient)
{
    //double cxy[2];
...
gboolean on_transmit_obs(gpointer _user)
{
    ...
    /*maplcm_floor_rect_list_t floor_rects;
    //    memcpy(&(floor_rects.rect_list), rects, sizeof(obs_rect_list_t));
    //    floor_rects.rect_list = rects;
    floor_rects.floor_ind = self->current_floor_ind;
    floor_rects.floor_no = self->current_floor_no;

    memset(&(floor_rects.rect_list), 0, sizeof(obs_rect_list_t));
    obs_rect_list_t *rects = &(floor_rects.rect_list); */
    obs_rect_list_t *rects = (obs_rect_list_t*) calloc(1, (pepper?pepper:1)*sizeof(obs_rect_list_t));
    rects->utime = bot_timestamp_now();
    rects->num_rects = self->rects->len*(pepper?pepper:1);
    rects->rects = (obs_rect_t*) calloc(rects->num_rects, sizeof(obs_rect_t));
    ...
    int rect_idx =0;
    for (unsigned int i = 0; i < self->rects->len; i++) {
        obs_rect_t *myrect = & g_array_index(self->rects, obs_rect_t,i);
        if (pepper) {
            ...
            for (int p=0;p<pepper;p++) {
                obs_rect_t *rect = &rects->rects[rect_idx++];
                double dx=0.0;
                double dy=0.0;
    ...
    obs_rect_list_t_publish(self->lcm, "SIM_RECTS", rects);

    //maplcm_floor_rect_list_t_publish(self->lcm, "FLOOR_SIM_RECTS", &floor_rects);
    free(rects->rects);
...
renderer_simobs_new (BotViewer *viewer, lcm_t *_lcm, BotParam *_param )
{
    ...
    self->rects = g_array_new(FALSE, FALSE, sizeof(obs_rect_t));
    self->edit_idx =-1;
    self->hover_idx =-1;
    ...
    */
    obs_rect_list_t_subscribe(self->lcm, "SIM_RECTS_NEW", on_sim_rects_new, self);

    maplcm_floor_status_msg_t_subscribe(self->lcm,"CURRENT_FLOOR_STATUS",floor_status_handler,
                                       self);
