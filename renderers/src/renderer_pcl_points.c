...
struct _RendererPCL {
    ...
    erlcm_segment_list_t *segments; 
    erlcm_normal_point_list_t *normals;

    erlcm_xyz_point_list_t *xyz_points;
    erlcm_xyzrgb_point_list_t *xyzrgb_points;
    erlcm_cylinder_list_t *cylinders; 
...
on_cylinder_models (const lcm_recv_buf_t *rbuf, const char *channel,
             const erlcm_cylinder_list_t *msg, void *user)
{
    ...
    if(self->cylinders){
        erlcm_cylinder_list_t_destroy(self->cylinders);
    }
    self->cylinders = erlcm_cylinder_list_t_copy(msg);
    self->have_data = 1;
             ...
             const erlcm_segment_list_t *msg, void *user)
{
    ...
    if(self->segments){
        erlcm_segment_list_t_destroy(self->segments);
    }
    self->segments = erlcm_segment_list_t_copy(msg);
    self->have_data = 1;
...
on_xyzrgb_points (const lcm_recv_buf_t *rbuf, const char *channel,
             const erlcm_xyzrgb_point_list_t *msg, void *user)
{
    RendererPCL *self = (RendererPCL *)user;
    g_assert(self);
    ...
    if(self->xyzrgb_points){
        erlcm_xyzrgb_point_list_t_destroy(self->xyzrgb_points);
    }
    self->xyzrgb_points = erlcm_xyzrgb_point_list_t_copy(msg);
    self->have_data = 1;    
    bot_viewer_request_redraw (self->viewer);
}
             ...
             const erlcm_normal_point_list_t *msg, void *user)
{
    ...
    if(self->normals){
        erlcm_normal_point_list_t_destroy(self->normals);
    }
    self->normals = erlcm_normal_point_list_t_copy(msg);
    self->have_data = 1;
    bot_viewer_request_redraw (self->viewer);
...
renderer_pcl_draw (BotViewer *viewer, BotRenderer *renderer)
{
    ...
    if(self->segments && bot_gtk_param_widget_get_bool (self->pw, DRAW_SEGMENTS)){
        ...
        for(int i=0; i< self->segments->no_segments; i++){
            erlcm_seg_point_list_t *seg_msg = &self->segments->segments[i];
    ...
    if(self->cylinders && bot_gtk_param_widget_get_bool (self->pw, DRAW_CYLINDERS)){
        ...
        for(int i=0; i< self->cylinders->count; i++){
            erlcm_cylinder_model_t *cyl = &self->cylinders->list[i];           
...
{    
    ...
    erlcm_xyz_point_list_t_subscribe(self->lcm, "PCL_XYZ_LIST", on_xyz_points, self);
    erlcm_xyzrgb_point_list_t_subscribe(self->lcm, "PCL_XYZRGB_LIST", on_xyzrgb_points, self);

    erlcm_cylinder_list_t_subscribe(self->lcm, "CYLINDER_LIST", on_cylinder_models, self);
