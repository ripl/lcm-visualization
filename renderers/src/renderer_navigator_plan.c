...
typedef struct _RendererNavigatorPlan {
    ...
    erlcm_goal_feasibility_querry_t *goal_querry_result;
...
void check_current_floor(RendererNavigatorPlan *s){
    ...
    erlcm_map_request_msg_t_publish(s->lc, "FLOOR_STATUS_REQUEST", &msg);
}
                      ...
                      gpointer user_data __attribute__ ((unused)))
{
    ...
    erlcm_tagged_node_t_publish(self->lc, "ADD_TAGGED_PLACE", &msg);
    free(msg.label);*/
    ...
    erlcm_place_node_t_publish(self->lc, "ADD_PLACE_NODE", &msg);
                      ...
                      gpointer user_data __attribute__ ((unused)))
{
    ...
    erlcm_portal_node_t_publish(self->lc, "ADD_PORTAL_NODE" , &msg);
...
static void  goal_querry_result_handler(const lcm_recv_buf_t *rbuf __attribute__((unused)), const char * channel __attribute__((unused)), const erlcm_goal_feasibility_querry_t * msg, void * user  __attribute__((unused)))
{
    RendererNavigatorPlan *self = (RendererNavigatorPlan*) user;
    if(self->goal_querry_result != NULL){
        erlcm_goal_feasibility_querry_t_destroy(self->goal_querry_result);
    }
    self->goal_querry_result = erlcm_goal_feasibility_querry_t_copy(msg);
...
static void on_param_widget_changed(BotGtkParamWidget *pw, const char *name, void *user)
{
    ...
    if(!strcmp(name, PLACE_NAV_GOAL)) {
        ...
        erlcm_goal_t_publish(self->lc, "CHECK_ELEVATOR", &ele_msg);
        ...
        msg.floor_no = self->current_floor_no;
        msg.std = 0;
        erlcm_place_node_t_publish(self->lc, "GOAL_NODE", &msg);
    }
