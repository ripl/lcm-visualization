...
typedef struct _RendererSatelliteView {
    ...
    ripl_gps_to_local_t_subscription_t * gps_subscr;
...
on_gps_to_local (const lcm_recv_buf_t * rbuf, const char * channel,
                 const ripl_gps_to_local_t * gl, void * user)
{
    RendererSatelliteView * self = (RendererSatelliteView *) user;
    g_mutex_lock(self->mutex);

    memcpy (&self->gps_to_local, gl, sizeof (ripl_gps_to_local_t));
...
void on_load_sat_activate(GtkMenuItem *mi, void *user)
{
    ...
    if (g_queue_is_empty (self->satellite_tiles)) {
        ...
        const char google_maps_path[256];
        sprintf (google_maps_path, "%s/satellite", getDataPath());
...
#undef SIZE
    ...
    ripl_gps_to_local_t_subscribe(self->lcm, "GPS_TO_LOCAL", on_gps_to_local, self);
