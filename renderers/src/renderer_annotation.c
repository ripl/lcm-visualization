...
typedef struct
{
    ...
    erlcm_annotation_t_subscription_t *sub;
  
    erlcm_annotation_t *annotation_last;
...
on_annotation(const lcm_recv_buf_t *rbuf, 
              const char *channel, const erlcm_annotation_t *msg, void *user)
{
    RendererAnnotation *self = (RendererAnnotation*)user;

    if (self->annotation_last)
        erlcm_annotation_t_destroy (self->annotation_last);

    self->annotation_last = erlcm_annotation_t_copy(msg);
...
BotRenderer *renderer_annotation_new(BotViewer *viewer)
{
    ...
    self->sub = erlcm_annotation_t_subscribe(self->lcm, "ANNOTATION", on_annotation, self);
