...
#include <carmen_utils/map3d_interface.h>
...
{
    static erlcm_multi_gridmap_t* staticmsg = NULL;
    if (staticmsg != NULL) {
        erlcm_multi_gridmap_t_destroy(staticmsg);
    }
    staticmsg = erlcm_multi_gridmap_t_copy(msg);
    ...
    /*erlcm_map_uncompress_lcm_map(map, staticmsg);
...
renderer_occupancy_map_new(BotViewer *viewer, int render_priority, BotParam * _param)
{
    ...
    erlcm_multi_gridmap_t_subscribe(self->lcm, "MULTI_FLOOR_MAPS", multi_gridmap_handler, self);
    erlcm_multi_gridmap_t_subscribe(self->lcm, "MMAP_SERVER", multi_gridmap_handler, self);
